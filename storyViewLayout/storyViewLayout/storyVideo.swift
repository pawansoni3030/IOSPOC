...
public class StoryVideo {
    ...
    public static let shared = StoryVideo()
...
//        }
        ...
        playerItemDidPlayToEndObserver = NotificationCenter.default.addObserver(
            ...
            queue: .main) { _ in
                let VC = vc as? storieViewController
                // "style_1" -> pager // "default" -> insta
                if VC?.storyStyle != "style_1"{
                    VC?.resetTimer()
                    VC?.moveRight()
                }
                self.restart()
